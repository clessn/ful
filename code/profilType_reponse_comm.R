...
ggplot(Graph, aes(y= prop, x= interaction(ses_female, ses_origin_qc))) + 
  ...
  geom_text(label = paste(round(Graph$prop, 2),"%"), nudge_y = 0.05, size = 5.5) + 
  ...
  labs(title = "Réponse positive à la dernière communication",
       subtitle = "Sont considérés comme ayant répondu positivement s'il y a un\ndon dans les 30 jours suivants la communication") +
  ylab("Proportion de réponses\npositives (%)") +
  xlab(element_blank()) +
...
ggsave("_SharedFolder_fondation-ulaval/graphs/profilsReponseComm.png",
       width = 12, height = 9)
